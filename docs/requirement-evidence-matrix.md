...
| 用户自行开发插件 | `sdk/plugin` 类型与构建器、ZIP 清单校验、项目/版本、私有安装、动态配置、优先级、启停、KV、运行记录；示例包由测试真实构建 | 39 项 Vitest 与生产 E2E 通过；插件无需部署独立进程或 Webhook |
...
| 安全 | AES-256-GCM 凭据、敏感设置不回传明文、服务端订单计价、支付幂等、HttpOnly 会话、CSRF 来源校验、限流、HMAC/Nonce、防重放、Webhook Ed25519、QuickJS 隔离与插件权限 | 50 项 Vitest、类型检查、干净 Lint、生产构建和 31 条 HTTP E2E 通过 |
